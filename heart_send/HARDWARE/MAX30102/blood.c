...
#include "usart.h"
#include "oled.h"
extern u8 data_buf[20];
extern u8 data_return;
...
void data_send(void)
	...
	data_buf[len++]=0x01; 
	  ...
	  data_buf[len++]=0xbb;
		data_buf[len++]=g_blooddata.heart; 
		data_buf[len++]=g_blooddata.heart; 			 
		 data_buf[len++]=((int)(g_blooddata.SpO2))>>8;
		 data_buf[len++]=g_blooddata.SpO2;
     data_buf[len++]=data_return;
		 data_buf[len++]=0xbb;
     data_buf[1]=len-2;
 	for(j=0;j<len;j++)//循环发送数据
	{
	  while(USART_GetFlagStatus(USART1,USART_FLAG_TC)==RESET);  //等待上次传输完成 
		USART_SendData(USART1,data_buf[j]); 	 //发送数据到串口1
	}
...
//	OLED_ShowNum(100,0,g_blooddata.heart,3,16);
	
//	OLED_ShowString(6,4,"Spo2:");
//	OLED_ShowNum(100,4,g_blooddata.SpO2,3,16);

...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueaccx != cur->valueaccx)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueaccx Changed\n");
            ret = 1;
        }
    }
    if(last->valueheart != cur->valueheart)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueheart Changed\n");
            ret = 1;
        }
    }
    if(last->valuespo2 != cur->valuespo2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuespo2 Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valueaccx = gizY2X(accx_RATIO,  accx_ADDITION, dataPoints->valueaccx); 
    devStatusPtr->valueheart = gizY2X(heart_RATIO,  heart_ADDITION, dataPoints->valueheart); 
    devStatusPtr->valuespo2 = gizY2X(spo2_RATIO,  spo2_ADDITION, dataPoints->valuespo2); 
